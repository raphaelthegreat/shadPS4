...
void PatchBufferSharp(IR::Block& block, IR::Inst& inst, Info& info, Descriptors& descriptors, const Profile& profile) {
...
void PatchGlobalDataShareAccess(IR::Block& block, IR::Inst& inst, Info& info,
    ...
    if (inst.GetOpcode() == IR::Opcode::DataAppend || inst.GetOpcode() == IR::Opcode::DataConsume) {
        ...
        // Patch instruction to GDS atomic.
        ...
        const u32 mask = (1u << std::countr_zero(profile.subgroup_size)) - 1;
        const IR::Value prev = is_append ? ir.BufferAtomicInc(handle, index, {})
                                         : ir.BufferAtomicDec(handle, index, {});
        inst.ReplaceUsesWithAndRemove(ir.BitwiseAnd(IR::U32{prev}, ir.Imm32(~mask)));
    } else {
    ...
    for (IR::Block* const block : program.blocks) {
        for (IR::Inst& inst : block->Instructions()) {
            if (IsBufferInstruction(inst)) {
                ...
                PatchGlobalDataShareAccess(*block, inst, info,
                                           profile, descriptors);
            }
        }
    }
