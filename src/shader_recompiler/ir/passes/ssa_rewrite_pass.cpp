...
using Variant =
    std::variant<IR::ScalarReg, IR::VectorReg, IR::VirtualReg, GotoVariable, MaskLaneVariable, ThreadBitScalar,
...
struct DefTable {
    ...
    const IR::Value& Def(IR::Block* block, IR::VirtualReg variable) {
        return reg_vars[variable.index][block];
    }
    void SetDef(IR::Block* block, IR::VirtualReg variable, const IR::Value& value) {
        reg_vars[variable.index].insert_or_assign(block, value);
    }
    ...
    std::unordered_map<u32, ValueMap> reg_vars;
    ValueMap scc_flag;
    ValueMap exec_flag;
    ValueMap vcc_flag;
    ...
    void FinalizePhiCandidates() {
        for (auto it = incomplete_phis.begin(); it != incomplete_phis.end(); ++it) {
            for (auto& [variant, phi] : it->second) {
                std::visit([&](auto& variable) { FinalizePhiCandidate(variable, *phi, it->first); }, variant);
            }
        }
    }
...
private:
    IR::Inst* GetUndefInst(IR::Opcode undef_opcode) {
        ...
        if (is_new) {
            IR::Inst* const undef{&*first_block->PrependNewInst(first_block->begin(), undef_opcode)};
            it->second = undef;
        }
        return it->second;
    }
    ...
    for (auto it1 = program.post_order_blocks.rbegin(); it1 != end; ++it1) {
        ...
        for (auto it = block->begin(); it != block->end(); ++it) {
            ...
            for (size_t arg_index = 0; arg_index < inst.NumArgs(); arg_index++) {
                ...
                if (arg.IsImmediate()) {
                    insert_point = arg_block->end();
                } else {
                    IR::Inst* arg_prod = arg.Inst();
                    if (arg_prod->GetParent() != arg_block) {
                        insert_point = arg_block->end();
                    } else {
                        insert_point = ++IR::Block::InstructionList::s_iterator_to(*arg_prod);
                    }
                }
                arg_block->PrependNewInst(insert_point, IR::Opcode::SetVirtualRegister, {IR::Value{reg}, arg});
            }
            IR::Inst* const new_inst{&*block->PrependNewInst(it, IR::Opcode::GetVirtualRegister, {IR::Value{reg}})};
