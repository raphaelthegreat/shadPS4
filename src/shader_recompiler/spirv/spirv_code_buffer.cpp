#include <cstring>
#include <limits>
#include "shader_recompiler/spirv/spirv_code_buffer.h"
...
SpirvCodeBuffer::SpirvCodeBuffer(uint32_t size) : m_ptr(size) {
    m_code.resize(size);
}

SpirvCodeBuffer::SpirvCodeBuffer(uint32_t size, const uint32_t* data) : m_ptr(size) {
    m_code.resize(size);
...
void SpirvCodeBuffer::putIns(spv::Op opCode, uint16_t wordCount) {
    this->putWord((static_cast<uint32_t>(opCode) << 0) | (static_cast<uint32_t>(wordCount) << 16));
}
...
void SpirvCodeBuffer::erase(size_t size) {
    m_code.erase(m_code.begin() + m_ptr, m_code.begin() + m_ptr + size);
}
