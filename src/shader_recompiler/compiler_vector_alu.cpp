...
void GcnCompiler::emitVectorAluCommon(const GcnShaderInstruction& ins) {
    ...
    case GcnOpcode::V_XOR_B32:
        dst.low.id = m_module.opBitwiseXor(typeId, src[0].low.id, src[1].low.id);
        break;
    // VectorConv
    case GcnOpcode::V_CVT_F32_U32:
        dst.low.id = m_module.opConvertUtoF(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_CVT_F32_I32:
        dst.low.id = m_module.opConvertStoF(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_CVT_U32_F32:
        dst.low.id = m_module.opConvertFtoU(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_CVT_I32_F32:
        dst.low.id = m_module.opConvertFtoS(typeId, src[0].low.id);
        break;
    ...
    case GcnOpcode::V_ADD_F32:
        dst.low.id = m_module.opFAdd(typeId, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_MAD_F32:
        dst.low.id = m_module.opFAdd(typeId, m_module.opFMul(typeId, src[0].low.id, src[1].low.id),
                                     src[2].low.id);
        break;
    case GcnOpcode::V_FMA_F32:
        dst.low.id = m_module.opFFma(typeId, src[0].low.id, src[1].low.id, src[2].low.id);
        break;
    case GcnOpcode::V_MUL_F32:
        dst.low.id = m_module.opFMul(typeId, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_MAC_F32: {
        auto vdst = emitVgprLoad(ins.dst[0]);
        dst.low.id =
            m_module.opFAdd(typeId, m_module.opFMul(typeId, src[0].low.id, src[1].low.id), vdst.id);
    } break;
    case GcnOpcode::V_SUB_F32:
        dst.low.id = m_module.opFSub(typeId, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_SUBREV_F32:
        dst.low.id = m_module.opFSub(typeId, src[1].low.id, src[0].low.id);
        break;
    case GcnOpcode::V_SUB_I32:
        dst.low.id = m_module.opISub(typeId, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_SUBREV_I32:
        dst.low.id = m_module.opISub(typeId, src[1].low.id, src[0].low.id);
        break;
    case GcnOpcode::V_MAX_F32:
        dst.low.id = m_module.opFMax(typeId, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_MIN_F32:
        dst.low.id = m_module.opFMin(typeId, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_MADMK_F32:
        dst.low.id = m_module.opFAdd(typeId, m_module.opFMul(typeId, src[0].low.id, src[2].low.id),
                                     src[1].low.id);
        break;
    ...
    case GcnOpcode::V_FLOOR_F32:
        dst.low.id = m_module.opFloor(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_FRACT_F32:
        dst.low.id = m_module.opFract(typeId, src[0].low.id);
        break;
    // VectorRegMov
    ...
    case GcnOpcode::V_ADDC_U32: {
        if (m_moduleInfo.options.separateSubgroup) {
            auto mask =
                emitCommonSystemValueLoad(GcnSystemValue::SubgroupEqMask, GcnRegMask::select(0));
            auto laneIdx = emitCommonSystemValueLoad(GcnSystemValue::SubgroupInvocationID,
                                                     GcnRegMask::select(0));
            ...
            dst.low.id = m_module.opIAdd(
                typeId, m_module.opIAdd(typeId, src[0].low.id, src[1].low.id), carry);
            // TODO:
    ...
    case GcnOpcode::V_MAD_I32_I24:
        dst.low.id = m_module.opIAdd(typeId, m_module.opIMul(typeId, src[0].low.id, src[1].low.id),
                                     src[2].low.id);
        break;
    case GcnOpcode::V_MUL_U32_U24: {
        u32 src0 = m_module.opBitFieldUExtract(typeId, src[0].low.id, m_module.constu32(0),
                                               m_module.constu32(24));
        u32 src1 = m_module.opBitFieldUExtract(typeId, src[1].low.id, m_module.constu32(0),
                                               m_module.constu32(24));
        dst.low.id = m_module.opIMul(typeId, src0, src1);
    } break;
    case GcnOpcode::V_MUL_I32_I24: {
        u32 src0 = m_module.opBitFieldSExtract(typeId, src[0].low.id, m_module.constu32(0),
                                               m_module.constu32(24));
        u32 src1 = m_module.opBitFieldSExtract(typeId, src[1].low.id, m_module.constu32(0),
                                               m_module.constu32(24));
        dst.low.id = m_module.opIMul(typeId, src0, src1);
    } break;
    ...
    case GcnOpcode::V_MUL_HI_I32: {
        std::array<u32, 2> members = {typeId, typeId};
        const u32 resultType = m_module.defStructType(members.size(), members.data());

        u32 mul = m_module.opSMulExtended(resultType, src[0].low.id, src[1].low.id);
        u32 memberIndex = 1;
    ...
    case GcnOpcode::V_MUL_HI_U32: {
        ...
        u32 mul = m_module.opUMulExtended(resultType, src[0].low.id, src[1].low.id);
        u32 memberIndex = 1;
        dst.low.id = m_module.opCompositeExtract(typeId, mul, 1, &memberIndex);
    } break;
    case GcnOpcode::V_MAX3_F32: {
        u32 max01 = m_module.opFMax(typeId, src[0].low.id, src[1].low.id);
        dst.low.id = m_module.opFMax(typeId, max01, src[2].low.id);
    } break;
    case GcnOpcode::V_MIN3_F32: {
        u32 min01 = m_module.opFMin(typeId, src[0].low.id, src[1].low.id);
        dst.low.id = m_module.opFMin(typeId, min01, src[2].low.id);
    } break;
    // VectorIntArith64
    case GcnOpcode::V_MAD_U64_U32: {
        ...
        u32 mul = m_module.opUMulExtended(resultType, src[0].low.id, src[1].low.id);
        u32 memberIndex = 0;
        u32 low = m_module.opCompositeExtract(typeId, mul, 1, &memberIndex);
        memberIndex = 1;
        u32 high = m_module.opCompositeExtract(typeId, mul, 1, &memberIndex);

        // add low part, this may produce carry out
        u32 add = m_module.opIAddCarry(resultType, low, src[2].low.id);
        // get low part result
        memberIndex = 0;
        dst.low.id = m_module.opCompositeExtract(typeId, add, 1, &memberIndex);
        // get carry
        memberIndex = 1;
        u32 carry = m_module.opCompositeExtract(typeId, add, 1, &memberIndex);
        // for high part, we need to add all 3 former results
        dst.high.id = m_module.opIAdd(typeId, m_module.opIAdd(typeId, high, src[2].high.id), carry);
    } break;
    // VectorFpTran32
    case GcnOpcode::V_RSQ_F32:
        dst.low.id = m_module.opInverseSqrt(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_SQRT_F32:
        dst.low.id = m_module.opSqrt(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_RCP_F32:
    case GcnOpcode::V_RCP_IFLAG_F32:
        dst.low.id = m_module.opFDiv(typeId, m_module.constf32(1.0), src[0].low.id);
        break;
    case GcnOpcode::V_LOG_F32:
        dst.low.id = m_module.opLog2(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_EXP_F32:
        dst.low.id = m_module.opExp2(typeId, src[0].low.id);
        break;
    case GcnOpcode::V_SIN_F32: {
        u32 radians = m_module.opFMul(typeId, src[0].low.id,
                                      m_module.constf32(static_cast<float>(2.0 * GcnPi)));
        dst.low.id = m_module.opSin(typeId, radians);
    } break;
    case GcnOpcode::V_COS_F32: {
        u32 radians = m_module.opFMul(typeId, src[0].low.id,
                                      m_module.constf32(static_cast<float>(2.0 * GcnPi)));
        dst.low.id = m_module.opCos(typeId, radians);
    } break;
    // VectorThreadMask
    case GcnOpcode::V_CNDMASK_B32: {
        ...
        auto mask = smask;
        mask.id = m_module.opBitwiseAnd(typeId, smask.id, eqMask.id);

        // Should we calculate the LSB of the mask?
        auto condition = emitRegisterZeroTest(mask, GcnZeroTest::TestNz);
        dst.low.id = m_module.opSelect(typeId, condition.id, src[1].low.id, src[0].low.id);
    } break;
    // VectorBitField32
    ...
    case GcnOpcode::V_LSHRREV_B32: {
        u32 shift = m_module.opBitFieldUExtract(typeId, src[0].low.id, m_module.constu32(0),
                                                m_module.constu32(5));
        dst.low.id = m_module.opShiftRightLogical(typeId, src[1].low.id, shift);
    } break;
    ...
    case GcnOpcode::V_LSHLREV_B32: {
        u32 shift = m_module.opBitFieldUExtract(typeId, src[0].low.id, m_module.constu32(0),
                                                m_module.constu32(5));
        dst.low.id = m_module.opShiftLeftLogical(typeId, src[1].low.id, shift);
    } break;
    case GcnOpcode::V_ASHR_I32: {
        u32 src1 = m_module.opBitcast(uintTypeId, src[1].low.id);
        u32 shift = m_module.opBitFieldUExtract(uintTypeId, src1, m_module.constu32(0),
                                                m_module.constu32(5));
        dst.low.id = m_module.opShiftRightArithmetic(typeId, src[0].low.id, shift);
    } break;
    case GcnOpcode::V_ASHRREV_I32: {
        u32 src0 = m_module.opBitcast(uintTypeId, src[0].low.id);
        u32 shift = m_module.opBitFieldUExtract(uintTypeId, src0, m_module.constu32(0),
                                                m_module.constu32(5));
        dst.low.id = m_module.opShiftRightArithmetic(typeId, src[1].low.id, shift);
    } break;
    case GcnOpcode::V_BFE_U32: {
        u32 shiftOffset = m_module.opBitFieldUExtract(typeId, src[1].low.id, m_module.constu32(0),
                                                      m_module.constu32(5));
        u32 shiftSize = m_module.opBitFieldUExtract(typeId, src[2].low.id, m_module.constu32(0),
                                                    m_module.constu32(5));
        u32 field = m_module.opShiftRightLogical(typeId, src[0].low.id, shiftOffset);
        u32 mask = m_module.opISub(
            typeId, m_module.opShiftLeftLogical(typeId, m_module.constu32(1), shiftSize),
            m_module.constu32(1));
    ...
    case GcnOpcode::V_SAD_U32: {
        ...
        u32 isSrc0Greater =
            m_module.opUGreaterThan(m_module.defBoolType(), src[0].low.id, src[1].low.id);
        ...
        // smaller one.
        u32 diff = m_module.opSelect(typeId, isSrc0Greater,
                                     m_module.opISub(typeId, src[0].low.id, src[1].low.id),
                                     m_module.opISub(typeId, src[1].low.id, src[0].low.id));
        dst.low.id = m_module.opIAdd(typeId, diff, src[2].low.id);
    } break;
    default:
        UNIMPLEMENTED();
        break;
...
void GcnCompiler::emitVectorMovRel(const GcnShaderInstruction& ins) {
    const u32 typeId = getScalarTypeId(ins.dst[0].type);
    ...
    case GcnOpcode::V_MOVRELD_B32: {
        ...
        u32 index = m_module.opIAdd(typeId, base, m0.id);
...
void GcnCompiler::emitVectorCmp(const GcnShaderInstruction& ins) {
    ...
    u32 condition = 0;
    u32 conditionType = m_module.defBoolType();
    ...
    case GcnOpcode::V_CMP_EQ_I32:
        condition = m_module.opIEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_GT_I32:
        condition = m_module.opSGreaterThan(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMPX_GT_U32:
        updateExec = true;
        [[fallthrough]];
    case GcnOpcode::V_CMP_GT_U32:
        condition = m_module.opUGreaterThan(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMPX_GE_U32:
        updateExec = true;
        [[fallthrough]];
    case GcnOpcode::V_CMP_GE_U32:
        condition = m_module.opUGreaterThanEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    ...
    case GcnOpcode::V_CMP_NLE_F32:
        condition = m_module.opFOrdGreaterThan(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_NGT_F32:
        condition = m_module.opFOrdLessThanEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_GE_F32:
        condition = m_module.opFOrdGreaterThanEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_EQ_F32:
        condition = m_module.opFOrdEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_NEQ_F32:
        condition = m_module.opFOrdNotEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_LE_I32:
        condition = m_module.opSLessThanEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_LE_U32:
        condition = m_module.opULessThanEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    ...
    case GcnOpcode::V_CMP_LT_U32:
        condition = m_module.opULessThan(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMPX_LT_I32:
        updateExec = true;
        [[fallthrough]];
    case GcnOpcode::V_CMP_LT_I32:
        condition = m_module.opSLessThan(conditionType, src[0].low.id, src[1].low.id);
        break;
    case GcnOpcode::V_CMP_LT_F32:
        condition = m_module.opFOrdLessThan(conditionType, src[0].low.id, src[1].low.id);
        break;
    ...
    case GcnOpcode::V_CMP_NE_I32:
        condition = m_module.opINotEqual(conditionType, src[0].low.id, src[1].low.id);
        break;
    default:
        UNIMPLEMENTED();
        break;
    ...
    ballot.id = m_module.opGroupNonUniformBallot(getVectorTypeId(ballot.type),
                                                 m_module.constu32(spv::ScopeSubgroup), condition);
...
void GcnCompiler::emitLaneReadFirst(const GcnShaderInstruction& ins) {
    ...
    // 64 lanes.
    dst.low.id = m_module.opGroupNonUniformBroadcastFirst(
        typeId, m_module.constu32(spv::ScopeSubgroup), src.low.id);
...
GcnRegisterValue GcnCompiler::emitCsLaneRead(const GcnRegisterValue& slane,
    ...
    const u32 utypeId = getScalarTypeId(GcnScalarType::Uint32);
    const u32 btypeId = m_module.defBoolType();

    auto subgroupId = emitCsSystemValueLoad(GcnSystemValue::SubgroupID, GcnRegMask::select(0));

    // detect if slane is less than 32, which means
    // it's in low 32 lanes
    u32 isLowGroup = m_module.opULessThan(btypeId, slane.id, m_module.constu32(32));
    // detect if current subgroup is even or odd
    u32 indexMod = m_module.opUMod(utypeId, subgroupId.id, m_module.constu32(2));
    u32 isEvenGroup = m_module.opIEqual(btypeId, indexMod, m_module.constu32(0));

    u32 isLowAndEven = m_module.opLogicalAnd(btypeId, isLowGroup, isEvenGroup);
    u32 isHighAndOdd = m_module.opLogicalAnd(btypeId, m_module.opLogicalNot(btypeId, isLowGroup),
                                             m_module.opLogicalNot(btypeId, isEvenGroup));

    // subtract 32 if slane is in high 32 lanes
    u32 laneIndex = m_module.opSelect(utypeId, isLowGroup, slane.id,
                                      m_module.opISub(utypeId, slane.id, m_module.constu32(32)));

    u32 needLoad = m_module.opLogicalOr(btypeId, isLowAndEven, isHighAndOdd);
    // Elect one lane to perform the store operation
    u32 election = m_module.opGroupNonUniformElect(btypeId, m_module.constu32(spv::ScopeSubgroup));

    u32 condition = m_module.opLogicalAnd(btypeId, needLoad, election);
    ...
    u32 value = m_module.opGroupNonUniformShuffle(utypeId, m_module.constu32(spv::ScopeSubgroup),
                                                  src.id, laneIndex);
    // store broadcast value to self slot
    const u32 ptrType = m_module.defPointerType(utypeId, spv::StorageClassWorkgroup);
    u32 ptrSelf = m_module.opAccessChain(ptrType, m_cs.crossGroupMemoryId, 1, &subgroupId.id);
    m_module.opStore(ptrSelf, value);

    // store broadcast value to neighbor slot
    u32 indexNeighbor = m_module.opSelect(
        utypeId, isEvenGroup, m_module.opIAdd(utypeId, subgroupId.id, m_module.constu32(1)),
        m_module.opISub(utypeId, subgroupId.id, m_module.constu32(1)));
    u32 ptrNeighbor = m_module.opAccessChain(ptrType, m_cs.crossGroupMemoryId, 1, &indexNeighbor);
    ...
    u32 ptr = m_module.opAccessChain(ptrType, m_cs.crossGroupMemoryId, 1, &subgroupId.id);
...
void GcnCompiler::emitLaneRead(const GcnShaderInstruction& ins) {
    ...
    const u32 utypeId = getVectorTypeId(dst.low.type);
    ...
    if (m_programInfo.type() == GcnProgramType::ComputeShader &&
        ...
        dst.low.id = m_module.opGroupNonUniformShuffle(
            utypeId, m_module.constu32(spv::ScopeSubgroup), src[0].low.id, src[1].low.id);
    }
...
void GcnCompiler::emitCubeCalculate(const GcnShaderInstruction& ins) {
    ...
    const u32 typeId = getVectorTypeId(dst.low.type);
    const u32 boolTypeId = m_module.defBoolType();
    const u32 intTypeId = getScalarTypeId(GcnScalarType::Sint32);
    auto op = ins.opcode;
...
/*
    void GcnCompiler::emitCubeCalculate(const GcnShaderInstruction& ins)
    {
        ...
        const u32 resultVar = emitNewVariable(info);

        u32 x    = src[0].low.id;
        u32 y    = src[1].low.id;
        u32 z    = src[2].low.id;
        u32 absX = emitRegisterAbsolute(src[0].low).id;
        u32 absY = emitRegisterAbsolute(src[1].low).id;
        u32 absZ = emitRegisterAbsolute(src[2].low).id;

        // if (abs(vz.f) >= abs(vx.f) && abs(vz.f) >= abs(vy.f))
        u32 zxCondition = m_module.opFOrdGreaterThanEqual(boolTypeId,
                                                               absZ, absX);
        u32 zyCondition = m_module.opFOrdGreaterThanEqual(boolTypeId,
                                                               absZ, absY);
        u32 zxyCondition = m_module.opLogicalAnd(boolTypeId,
                                                      zxCondition, zyCondition);

        u32 endLabel = m_module.allocateId();
        m_module.opSelectionMerge(endLabel, spv::SelectionControlMaskNone);

        u32 zxyLabel  = m_module.allocateId();
        u32 yxIfLabel = m_module.allocateId();
        ...
        switch (op)
        {
            case GcnOpcode::V_CUBEID_F32:
            {
                u32 condition = m_module.opFOrdLessThan(boolTypeId,
                                                             z, m_module.constf32(0.0));
                u32 value     = m_module.opSelect(typeId,
                                                       condition,
            ...
            case GcnOpcode::V_CUBETC_F32:
            {
                u32 value = m_module.opFNegate(typeId, y);
                m_module.opStore(resultVar, value);
            }
                break;
            case GcnOpcode::V_CUBEMA_F32:
            {
                u32 value = m_module.opFMul(typeId,
        ...
        u32 yxCondition = m_module.opFOrdGreaterThanEqual(boolTypeId,
                                                               absY, absX);
        // A empty block right before end block used to build merge block
        u32 beforeEndLabel = m_module.allocateId();
        m_module.opSelectionMerge(beforeEndLabel, spv::SelectionControlMaskNone);

        u32 yxLabel = m_module.allocateId();
        u32 xLabel  = m_module.allocateId();
