...
#include "shader_recompiler/control_flow/control_flow_graph.h"
...
public:
    ...
    bool contains(GcnCfgVertex vtx) const {
        return std::find(m_vertices.begin(), m_vertices.end(), vtx) != m_vertices.end();
    }
...
class GcnLoopInfo {
    ...
    public:
        LoopVisitor(const std::unordered_set<GcnCfgVertex>& headers, std::vector<GcnLoop>& loops);
...
private:
    ...
    std::unordered_set<GcnCfgVertex> findLoopHeaders();
