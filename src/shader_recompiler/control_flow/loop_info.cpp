...
namespace boost {
template <typename G>
inline void renumber_vertex_indices(G& g) {
    throw;
}
} // namespace boost
...
GcnLoopInfo::LoopVisitor::LoopVisitor(const std::unordered_set<GcnCfgVertex>& headers,
                                      std::vector<GcnLoop>& loops)
    : m_headers(headers), m_loops(loops) {}
...
void GcnLoopInfo::LoopVisitor::back_edge(GcnCfgEdge edge, const GcnControlFlowGraph& g) {
    auto target = boost::target(edge, g);
    ASSERT_MSG(m_loopStack.empty() || target == m_loopStack.back()->getHeader(),
               "loop stack not match.");
...
GcnLoopInfo::GcnLoopInfo(const GcnControlFlowGraph& cfg) : m_cfg(cfg) {
    detectLoops();
    detectVertexMaping();
}
...
bool GcnLoopInfo::isLoopHeader(GcnCfgVertex vtx) const {
    ...
    return (loop != nullptr) && (loop->getHeader() == vtx);
}
...
void GcnLoopInfo::detectLoops() {
    ...
    // Note this reserve is critical, it makes sure the vector will not reallocate
    // memory, such that we can safely save pointers to the element in the vector.
