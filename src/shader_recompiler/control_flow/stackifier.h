...
#include "shader_recompiler/control_flow/divergent_flow.h"
#include "shader_recompiler/control_flow/dominator_tree.h"
#include "shader_recompiler/control_flow/loop_info.h"
...
class GcnTokenListBuilder {
    struct StackElement {
        enum Kind { Immediate, Delayed };

        StackElement(GcnCfgVertex vtx, Kind kind) : m_vertex(vtx), m_kind(kind) {}
...
public:
    GcnTokenListBuilder(const GcnControlFlowGraph& cfg, GcnTokenFactory& factory);
    ~GcnTokenListBuilder();
...
private:
    ...
    u32 getNumForwardPreds(GcnCfgVertex vtx);
...
private:
    ...
    std::unordered_map<GcnCfgVertex, u32> m_visitedCounts;
    std::unordered_map<GcnCfgVertex, std::vector<GcnCfgVertex>> m_delayedQueues;
    std::unordered_map<const GcnLoop*, u32> m_loopCounts;
...
public:
    GcnTokenListOptimizer(const GcnControlFlowGraph& cfg, GcnTokenFactory& factory);
...
private:
    ...
    template <u32 Kind, typename FnType>
    void for_each_kind(GcnTokenList::iterator begin, GcnTokenList::iterator end, FnType func);
...
private:
    ...
    std::vector<u32> m_removeFlagStack;
};
...
class GcnGotoEliminator {
    struct GotoInfo {
        ...
        u32 levelDistance; // level(goto) - level(label)
    };
...
public:
    GcnGotoEliminator(const GcnControlFlowGraph& cfg, GcnTokenFactory& factory);
    ~GcnGotoEliminator();
...
private:
    ...
    bool isInRange(GcnToken* begin, GcnToken* end, GcnToken* target);
    bool isInRange(GcnTokenList::iterator begin, GcnTokenList::iterator end,
...
class GcnStackifier {
public:
    GcnStackifier(const GcnControlFlowGraph& cfg);
    ~GcnStackifier();
...
private:
    bool isIrreducible();
    bool isMultiEntryLoop(const std::unordered_set<GcnCfgVertex>& loop);
