...
struct PushData {
    ...
    static constexpr size_t PushDataOffset = 5;
    static constexpr size_t PushDwordsPerStage = 4;

    u32 step0;
    u32 step1;
    std::array<u8, 48> buf_offsets;
    std::array<u32, 8> ud_data;
...
struct Info {
    ...
    struct UserDataMask {
        ...
        u32 Offset(IR::ScalarReg reg) const noexcept {
            const u32 index = std::popcount(u32{mask} & ((1U << static_cast<u32>(reg)) - 1));
            ASSERT(index < PushData::PushDwordsPerStage);
            return index;
        }
