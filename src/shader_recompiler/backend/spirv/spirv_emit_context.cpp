...
void EmitContext::DefineInputs() {
    ...
    if (info.loads.GetAny(IR::Attribute::SubgroupEqMask)) {
        subgroup_eq_mask = DefineVariable(U32[4], spv::BuiltIn::SubgroupEqMask, spv::StorageClass::Input);
    }
    if (info.loads.GetAny(IR::Attribute::SubgroupLtMask)) {
        subgroup_lt_mask = DefineVariable(U32[4], spv::BuiltIn::SubgroupLtMask, spv::StorageClass::Input);
    }
    switch (l_stage) {
