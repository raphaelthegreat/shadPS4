...
Id EmitGetAttributeU32(EmitContext& ctx, IR::Attribute attr, u32 comp) {
    ...
    case IR::Attribute::SubgroupEqMask:
        return ctx.OpLoad(ctx.U32[1], ctx.OpAccessChain(ctx.input_u32, ctx.subgroup_eq_mask, ctx.ConstU32(comp)));
    case IR::Attribute::SubgroupLtMask:
        return ctx.OpLoad(ctx.U32[1], ctx.OpAccessChain(ctx.input_u32, ctx.subgroup_lt_mask, ctx.ConstU32(comp)));
