...
void GcnCompiler::compileTokenEnd(const GcnToken& token) {
    ...
    case GcnTokenKind::Block:
        ...
        break;
    default:
        UNREACHABLE_MSG("GcnCompiler: Invalid header kind: {}", (u32)kind);
        break;
    }
}
...
void GcnCompiler::compileTokenVariable(const GcnToken& token) {
    ...
    if (iter == m_tokenVariables.end()) {
        ...
        u32 varId = emitNewVariable(info);
...
void GcnCompiler::compileTokenSetValue(const GcnToken& token) {
    ...
    ASSERT_MSG(iter != m_tokenVariables.end(), "Value set before define.");
...
u32 GcnCompiler::emitComputeDivergence() {
    ...
    const u32 ftypeId = getScalarTypeId(GcnScalarType::Float32);
    ...
    u32 labelEven = m_module.allocateId();
    u32 labelOdd = m_module.allocateId();
    u32 labelMerge = m_module.allocateId();
    ...
    u32 exec = m_module.opPhi(utypeId, phiLabels.size(), phiLabels.data());
    ...
    u32 result = m_module.opINotEqual(btypeId, value, m_module.constu32(0));
    return result;
}

u32 GcnCompiler::emitControlFlowDivergence() {
    u32 result = 0;
    ...
    if (m_moduleInfo.options.separateSubgroup) {
        ...
        auto mask =
            emitCommonSystemValueLoad(GcnSystemValue::SubgroupEqMask, GcnRegMask::firstN(2));
...
u32 GcnCompiler::emitControlFlowCondition(const GcnTokenCondition& condition) {
    u32 result = 0;
    ...
    auto loadCompareValue = [&]() {
        ...
        u32 lhs = m_module.opLoad(getVectorTypeId(condVar.type), condVar.id);
    ...
    case GcnConditionOp::Scc0: {
        u32 scc = m_module.opLoad(typeId, m_state.scc.id);
...
void GcnCompiler::emitControlFlowIf(const GcnToken& token) {
    ...
    u32 conditionId = emitControlFlowCondition(condition);
...
void GcnCompiler::emitControlFlowElse(const GcnToken& token) {
    if (m_controlFlowStack.size() == 0 || m_controlFlowStack.back().type != GcnCfgBlockType::If ||
        m_controlFlowStack.back().b_if.labelElse != 0) {
        UNREACHABLE_MSG("GcnCompiler: 'Else' without 'If' found");
    }
...
void GcnCompiler::emitControlFlowEndIf(const GcnToken& token) {
    if (m_controlFlowStack.size() == 0 || m_controlFlowStack.back().type != GcnCfgBlockType::If) {
        UNREACHABLE_MSG("GcnCompiler: 'EndIf' without 'If' found");
    }
    ...
    u32 labelMerge = m_blockTerminators.empty() ? block.b_if.labelEnd : m_blockTerminators.back();
    ...
    m_module.opBranchConditional(block.b_if.conditionId, block.b_if.labelIf,
                                 block.b_if.labelElse != 0 ? block.b_if.labelElse : labelMerge);
...
void GcnCompiler::emitControlFlowLoop(const GcnToken& token) {
    ...
    m_module.opLoopMerge(block.b_loop.labelBreak, block.b_loop.labelContinue,
    ...
    u32 labelTrue = m_module.allocateId();
    m_module.opBranchConditional(m_module.constBool(true), labelTrue, block.b_loop.labelBreak);
    m_module.opLabel(labelTrue);
}

void GcnCompiler::emitControlFlowEndLoop(const GcnToken& token) {
    if (m_controlFlowStack.size() == 0 || m_controlFlowStack.back().type != GcnCfgBlockType::Loop) {
        UNREACHABLE_MSG("GcnCompiler: 'EndLoop' without 'Loop' found");
    }
...
void GcnCompiler::emitControlFlowEndBlock(const GcnToken& token) {
    ...
    m_module.opBranchConditional(m_module.constBool(false), block.b_loop.labelHeader,
...
void GcnCompiler::emitControlFlowBreak(const GcnToken& token) {
    ...
    if (cfgBlock == nullptr) {
        UNREACHABLE_MSG("GcnCompiler: 'Break' or 'Continue' outside 'Loop' or 'Switch' found");
    }

    m_module.opBranch(isBreak ? cfgBlock->b_loop.labelBreak : cfgBlock->b_loop.labelContinue);

    // Subsequent instructions assume that there is an open block
    const u32 labelId = m_module.allocateId();
...
void GcnCompiler::emitControlFlowReturn() {
    if (m_controlFlowStack.size() != 0) {
        ...
        if (m_blockTerminators.empty()) {
            u32 labelId = m_module.allocateId();
...
GcnCfgBlock* GcnCompiler::cfgFindBlock(const std::initializer_list<GcnCfgBlockType>& types) {
    for (auto cur = m_controlFlowStack.rbegin(); cur != m_controlFlowStack.rend(); cur++) {
        for (auto type : types) {
            if (cur->type == type)
                return &(*cur);
        }
...
void GcnCompiler::emitUpdateScc(GcnRegisterValuePair& dst, GcnScalarType dstType) {
    // TODO:
    ...
    if (isDoubleType(dstType)) {
        ...
        result.id = m_module.opLogicalOr(m_module.defBoolType(), result.id, resultHigh.id);
    }
