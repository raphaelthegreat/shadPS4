...
GcnAnalyzer::GcnAnalyzer(const GcnProgramInfo& programInfo, GcnAnalysisInfo& analysis)
    : m_analysis(&analysis), m_programInfo(programInfo) {}

GcnAnalyzer::~GcnAnalyzer() = default;

void GcnAnalyzer::processInstruction(const GcnShaderInstruction& ins) {
...
void GcnAnalyzer::analyzeExp(const GcnShaderInstruction& ins) {
    ...
    if (exp.target == GcnExportTarget::Param) {
        ...
        u32 paramIndex = exp.control.target - GcnExpParam0;
        ...
        u32 mrtIndex = exp.control.target;
...
void GcnAnalyzer::analyzeLane(const GcnShaderInstruction& ins) {
    ...
    if (m_programInfo.type() == GcnProgramType::ComputeShader &&
        (op == GcnOpcode::V_READLANE_B32 || op == GcnOpcode::V_WRITELANE_B32)) {
