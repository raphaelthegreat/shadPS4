...
bool isForkInstruction(const GcnShaderInstruction& ins) {
    return ins.opcode == GcnOpcode::S_CBRANCH_I_FORK || ins.opcode == GcnOpcode::S_CBRANCH_G_FORK ||
...
bool isTerminateInstruction(const GcnShaderInstruction& ins) {
    return isBranchInstruction(ins) || isForkInstruction(ins) || ins.opcode == GcnOpcode::S_ENDPGM;
}

u32 calculateBranchTarget(u32 pc, const GcnShaderInstruction& ins) {
    auto sopp = gcnInstructionAs<GcnShaderInstSOPP>(ins);
    u32 target = static_cast<u32>(static_cast<int32_t>(pc) +
                                  static_cast<int16_t>(sopp.control.simm << 2) + 4);
    return target;
}

bool isVop3BEncoding(GcnOpcode opcode) {
    return opcode == GcnOpcode::V_ADD_I32 || opcode == GcnOpcode::V_ADDC_U32 ||
           opcode == GcnOpcode::V_SUB_I32 || opcode == GcnOpcode::V_SUBB_U32 ||
           opcode == GcnOpcode::V_SUBREV_I32 || opcode == GcnOpcode::V_SUBBREV_U32 ||
           opcode == GcnOpcode::V_DIV_SCALE_F32 || opcode == GcnOpcode::V_DIV_SCALE_F64 ||
           opcode == GcnOpcode::V_MAD_U64_U32 || opcode == GcnOpcode::V_MAD_I64_I32;
}

bool isImageAccessSampling(const GcnShaderInstruction& ins) {
    auto op = ins.opcode;
    return op >= GcnOpcode::IMAGE_SAMPLE && op <= GcnOpcode::IMAGE_SAMPLE_C_CD_CL_O &&
           op != GcnOpcode::IMAGE_GET_LOD;
}

bool isImageAccessNoSampling(const GcnShaderInstruction& ins) {
    auto op = ins.opcode;
    return op >= GcnOpcode::IMAGE_LOAD && op <= GcnOpcode::IMAGE_ATOMIC_FMAX &&
           op != GcnOpcode::IMAGE_GET_RESINFO;
}

bool isUavReadAccess(const GcnShaderInstruction& ins) {
    auto op = ins.opcode;
    return op >= GcnOpcode::IMAGE_LOAD && op <= GcnOpcode::IMAGE_LOAD_MIP_PCK_SGN;
}
