...
public:
    ...
    void VerifyNode(N* node, u64 min_key = 0, u64 max_key = ~0ULL) {
        ...
        ASSERT_MSG(node->prev != nullptr, "null prev");
        ASSERT_MSG(node->next != nullptr, "null next");
        ASSERT_MSG(Start(node) >= min_key, "BST violation: start {:#x} < min {:#x}", Start(node),
                   min_key);
        ASSERT_MSG(Start(node) < max_key, "BST violation: start {:#x} >= max {:#x}", Start(node),
                   max_key);
    ...
    std::pair<iterator, bool> LookupEntry(u64 addr) {
        ...
        if (addr < Start(cur) && cur->prev != m_header) {
            ...
            ASSERT_MSG(End(cur->prev) <= addr,
                       "splay landed wrong: addr={:#x} cur=[{:#x},{:#x}) prev=[{:#x},{:#x})", addr,
                       Start(cur), End(cur), Start(cur->prev), End(cur->prev));
        }
