...
static u64 g_load_addr = Common::SYSTEM_RESERVED + Common::CODE_BASE_OFFSET;
...
static std::string encodeId(u64 n_val) {
    std::string enc;
    constexpr char codes[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-";
    if (n_val < 0x40u) {
        enc += codes[n_val];
    }
    else {
        if (n_val < 0x1000u) {
            enc += codes[static_cast<u16>(n_val >> 6u) & 0x3fu];
            enc += codes[n_val & 0x3fu];
        } else {
            enc += codes[static_cast<u16>(n_val >> 12u) & 0x3fu];
            enc += codes[static_cast<u16>(n_val >> 6u) & 0x3fu];
            enc += codes[n_val & 0x3fu];
        }
    }
...
void Linker::LoadModuleToMemory(Module* m) {
    ...
    // Align base size to 0x1000 block size (TODO is that the default block size or it can be changed?
    m->aligned_base_size = (base_size & ~(static_cast<u64>(0x1000) - 1)) + 0x1000;
    m->base_virtual_addr = Common::memory_alloc(g_load_addr, m->aligned_base_size, Common::MemoryMode::ExecuteReadWrite);
        ...
        switch(elf_pheader[i].p_type) {
			...
			case PT_SCE_RELRO:
                if (elf_pheader[i].p_memsz != 0) {
                    const u64 segment_addr = elf_pheader[i].p_vaddr + m->base_virtual_addr;
                    const u64 segment_file_size = elf_pheader[i].p_filesz;
                    const u64 segment_memory_size = getAlignedSize(elf_pheader[i]);
                    const auto segment_mode = m->elf.ElfPheaderFlagsStr(elf_pheader[i].p_flags);
...
void Linker::LoadDynamicInfo(Module* m)
{
    ...
    for (const auto* dyn = reinterpret_cast<elf_dynamic*>(m->m_dynamic.data()); dyn->d_tag != DT_NULL; dyn++) {
		...
		case DT_SCE_HASH: //Offset of the hash table.
            m->dynamic_info->hash_table = reinterpret_cast<void*>(reinterpret_cast<u8*>(m->m_dynamic_data.data()) + dyn->d_un.d_ptr);
			break;
		...
		case DT_SCE_STRTAB://Offset of the string table. 
            m->dynamic_info->str_table = reinterpret_cast<char*>(reinterpret_cast<u8*>(m->m_dynamic_data.data()) + dyn->d_un.d_ptr);
			break;
		...
		case DT_SCE_SYMTAB://Offset of the symbol table.
            m->dynamic_info->symbol_table = reinterpret_cast<elf_symbol*>(reinterpret_cast<u8*>(m->m_dynamic_data.data()) + dyn->d_un.d_ptr);
			break;
        ...
        case DT_SCE_JMPREL: // Offset of the table containing jump slots.
            m->dynamic_info->jmp_relocation_table = reinterpret_cast<elf_relocation*>(reinterpret_cast<u8*>(m->m_dynamic_data.data()) + dyn->d_un.d_ptr);
			break;
        ...
        case DT_SCE_RELA: // Offset of the relocation table.
            m->dynamic_info->relocation_table = reinterpret_cast<elf_relocation*>(reinterpret_cast<u8*>(m->m_dynamic_data.data()) + dyn->d_un.d_ptr);
			break;
...
void Linker::LoadSymbols(Module* m) {
    if (!m->dynamic_info->symbol_table || !m->dynamic_info->str_table || m->dynamic_info->symbol_table_total_size == 0) {
	...
	for (auto* sym = m->dynamic_info->symbol_table;
        ...
        auto ids = Common::splitString(id, '#');
        if (ids.size() == 3) { // Symbols are 3 parts name, library, module
            ...
            if (library != nullptr || module != nullptr) {
                ...
                const bool is_sym_export = sym->st_value != 0;
				std::string nidName = "";
...
{
    const auto relocate = [](u32 idx, elf_relocation* rel, Module* m, bool isJmpRel) {
        ...
        u8 rel_sym_type = 0;
        std::string rel_name;
        u8 rel_bind_type = -1;  //-1 means it didn't resolve
        ...
        if (rel_isResolved) {
            Common::memory_patch(rel_virtual_addr, rel_value);
        } else {
	...
	u32 idx = 0;
    for (auto* rel = m->dynamic_info->relocation_table; reinterpret_cast<u8*>(rel) < reinterpret_cast<u8*>(m->dynamic_info->relocation_table) + m->dynamic_info->relocation_table_size; rel++, idx++) {
		relocate(idx, rel, m, false);
	}
	idx = 0;
    for (auto* rel = m->dynamic_info->jmp_relocation_table; reinterpret_cast<u8*>(rel) < reinterpret_cast<u8*>(m->dynamic_info->jmp_relocation_table) + m->dynamic_info->jmp_relocation_table_size; rel++, idx++) {
		relocate(idx, rel, m, true);
	}
}

void Linker::Resolve(const std::string& name, int sym_type, Module* m, Loader::SymbolRecord* return_info) {
    ...
    if (rec) {
        *return_info = *rec;
        ...
        auto aeronid = AeroLib::findByNid(sr.name);
        if (aeronid) {
...
void Linker::Execute() {
    ...
    const u64 base_addr = m_modules.at(0).elf.GetElfEntry()+m_modules.at(0).base_virtual_addr;
    ...
    asm volatile (
        ...
        :
        : "r"(base_addr), "r"(&p), "r"(programExitFunc)
        : "rax", "rsi", "rdi", "rsp", "rbp"
        );
}
