    ...
    struct Bitmap {
        ...
        constexpr void Allocate(u32 count, u32* out_blocks) {
            while (count && bits_l2) {
                ...
                const u32 l0_index = std::countr_zero(bits_l1[l1_index]) | (l1_index << LEVEL_SHIFT);

                u32 entry = bits_l0[l0_index];
                do {
        ...
        constexpr void InsertLevel(u32 start_bit, u32 num_bits, u64* bits) {
            ...
            bits[start_index] |= (~u64{0} >> (64 - std::min(num_bits, 64u))) << (start_bit & LEVEL_MASK);
