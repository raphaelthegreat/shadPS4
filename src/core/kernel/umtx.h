...
struct umutex {
    volatile u_long m_owner;	/* Owner of the mutex */
    u_int m_flags;	/* Flags of the mutex */
    u_int m_ceilings[2];	/* Priority protect ceiling */
    u_int m_spare[4];
};

struct ucond {
    volatile u_int c_has_waiters;	/* Has waiters in kernel */
    u_int c_flags;	/* Flags of the condition variable */
    u_int c_clockid;	/* Clock id */
    u_int c_spare[1];	/* Spare space */
};

struct urwlock {
    volatile int rw_state;
    u_int rw_flags;
    u_int rw_blocked_readers;
    u_int rw_blocked_writers;
    u_int rw_spare[4];
};

struct _usem {
    volatile u_int _has_waiters;
    volatile u_int _count;
    u_int _flags;
};
