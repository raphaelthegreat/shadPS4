...
bool Blockpool::Commit(u32 count, bool onion, bool writeback, u32* out_blocks) {
    ...
    while (count) {
        if (onion) {
            if (count <= cached.available_blocks) {
                cached.Allocate(count, out_blocks);
            } else {
                const u32 flushed_count = count - cached.available_blocks;
                cached.Allocate(cached.available_blocks, out_blocks);
                flushed.Allocate(flushed_count, out_blocks);
            }
            break;
        } else {
            ...
            if (cached.available_blocks < 32) {
                ...
                cached.Allocate(cached_count, out_blocks);
                break;
            }
        }
        Flush();
...
void Blockpool::Query(VAddr addr, VAddr base, u64 size, const DmemBlock* blocks, ::Libraries::Kernel::OrbisVirtualQueryInfo* info) {
    u32 start = 0;
    u32 end = 0;
    ...
    if (!vm_block.valid) {
        ...
        while (start_block > block_min && blocks[start_block].Props() == blocks[start_block - 1].Props()) {
            --start_block;
        }
        while (end_block < block_max && blocks[end_block].Props() == blocks[end_block + 1].Props()) {
            ++end_block;
        }
    }
