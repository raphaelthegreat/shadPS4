...
bool MemoryManager::TryWriteBacking(void* address, const void* data, u64 size) {
    ...
    while (current_vma->second.Overlaps(virtual_addr, size)) {
        if (!current_vma->second.HasPhysicalBacking()) {
            break;
        }
...
PAddr MemoryManager::Allocate(PAddr search_start, PAddr search_end, u64 size, u64 alignment,
    ...
    PAddr mapping_start = Common::AlignUp(search_start, alignment);
    PAddr mapping_end = mapping_start + size;

    // Find the first free, large enough dmem area in the range.
    while (dmem_area->second.type != PhysicalMemoryType::Free ||
    ...
    area.type = PhysicalMemoryType::Allocated;
...
s32 MemoryManager::Free(PAddr phys_addr, u64 size, bool is_checked) {
    ...
    for (; phys_handle != dmem_map.end(); phys_handle++) {
        ...
        if (dmem_area.type == PhysicalMemoryType::Free) {
            if (is_checked) {
    ...
    for (auto& [phys_addr, size] : free_list) {
        ...
        new_dmem_area.type = PhysicalMemoryType::Free;
...
s32 MemoryManager::PoolCommit(VAddr virtual_addr, u64 size, MemoryProt prot, s32 mtype) {
    ...
    const auto map_memory = [&] {
        ...
        impl.Map(virtual_addr + Blockpool::ToBytes(start_block),
                 Blockpool::ToBytes(pending_blocks),
...
s32 MemoryManager::MapMemory(void** out_addr, VAddr virtual_addr, u64 size, MemoryProt prot,
    ...
    if (phys_addr != -1) {
        ...
        while (dmem_area != dmem_map.end() && dmem_area->second.base < phys_addr + size) {
            ...
            if (dmem_area->second.type != PhysicalMemoryType::Allocated &&
                dmem_area->second.type != PhysicalMemoryType::Mapped) {
                LOG_ERROR(Kernel_Vmm, "Unable to map {:#x} bytes at physical address {:#x}", size,
                          phys_addr);
                return ORBIS_KERNEL_ERROR_ENOMEM;
            }
            ...
            if (validate_dmem && dmem_area->second.type == PhysicalMemoryType::Mapped) {
                LOG_ERROR(Kernel_Vmm, "Unable to map {:#x} bytes at physical address {:#x}", size,
                          phys_addr);
    ...
    if (type == VMAType::Flexible) {
        ...
        while (handle != fmem_map.end() &&
               (handle->second.type != PhysicalMemoryType::Free || handle->second.size < size)) {
            handle++;
        }
        // Some games will end up fragmenting the flexible address space.
        ASSERT_MSG(handle != fmem_map.end() && handle->second.type == PhysicalMemoryType::Free,
                   "No suitable physical memory areas to map");

        // We'll use the start of this area as the physical backing for this mapping.
        const auto new_fmem_handle = CarvePhysArea(fmem_map, handle->second.base, size);
        auto& new_fmem_area = new_fmem_handle->second;
        new_fmem_area.type = PhysicalMemoryType::Flexible;
...
s32 MemoryManager::MapFile(void** out_addr, VAddr virtual_addr, u64 size, MemoryProt prot,
    ...
    ASSERT_MSG(prot < MemoryProt::GpuRead, "Files cannot be mapped to GPU memory");
...
u64 MemoryManager::UnmapBytesFromEntry(VAddr virtual_addr, VirtualMemoryArea vma_base, u64 size) {
    ...
    const bool has_backing = vma_base.HasPhysicalBacking() || type == VMAType::File;
    ...
    if (type == VMAType::Direct) {
        ...
        while (dmem_handle != dmem_map.end() && remaining_size > 0) {
            ...
            dmem_area.type = PhysicalMemoryType::Allocated;
    ...
    if (type == VMAType::Flexible) {
        ...
        const auto new_fmem_handle = CarvePhysArea(fmem_map, unmap_phys_base, adjusted_size);
        auto& new_fmem_area = new_fmem_handle->second;
        new_fmem_area.type = PhysicalMemoryType::Free;
        MergeAdjacent(fmem_map, new_fmem_handle);
    }
...
s32 MemoryManager::QueryProtection(VAddr addr, void** start, void** end, u32* prot) {
    ...
    if (vma.type != VMAType::Pooled) {
        ...
        blockpool.Query(addr, vma.base, vma.size, vma.blocks.data(), &info);
        if (start != nullptr) {
            *start = reinterpret_cast<void*>(info.start);
        }
        if (end != nullptr) {
            *end = reinterpret_cast<void*>(info.end);
        }
        if (prot != nullptr) {
            *prot = static_cast<u32>(info.protection);
        }
    }
...
s32 MemoryManager::VirtualQuery(VAddr addr, s32 flags,
    ...
    if (vma.type == VMAType::Direct) {
        ...
        info->memory_type = vma.memory_type;
    }
...
s32 MemoryManager::DirectMemoryQuery(PAddr addr, bool find_next,
    ...
    while (dmem_area != dmem_map.end() && dmem_area->second.type == PhysicalMemoryType::Free &&
           find_next) {
        dmem_area++;
    }

    if (dmem_area == dmem_map.end() || dmem_area->second.type == PhysicalMemoryType::Free) {
        LOG_WARNING(Kernel_Vmm, "Unable to find allocated direct memory region to query!");
        return ORBIS_KERNEL_ERROR_EACCES;
    }
    ...
    while (dmem_area != dmem_map.end() && dmem_area->second.memory_type == out_info->memoryType &&
           (dmem_area->second.type == PhysicalMemoryType::Mapped ||
            dmem_area->second.type == PhysicalMemoryType::Allocated)) {
...
s32 MemoryManager::DirectQueryAvailable(PAddr search_start, PAddr search_end, u64 alignment,
    ...
    while (dmem_area != dmem_map.end()) {
        if (dmem_area->second.type != PhysicalMemoryType::Free) {
            dmem_area++;
            continue;
        }
