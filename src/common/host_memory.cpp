...
static void* ChooseVirtualBase(size_t virtual_size) {
    constexpr uintptr_t Map39BitSize = (1ULL << 40);
    constexpr uintptr_t Map22BitSize = (1ULL << 22);
    ...
    const size_t lower = Map22BitSize / HugePageSize;
...
public:
    explicit Impl(size_t backing_size_, size_t virtual_size_)
        ...
        if (page_size != 0x1000) {
            LOG_CRITICAL(Common, "page size {:#x} is incompatible with 4K paging", page_size);
            throw std::bad_alloc{};
        }
        ...
        if (fd < 0) {
            LOG_CRITICAL(Common, "memfd_create failed: {}", strerror(errno));
            throw std::bad_alloc{};
        }
        ...
        if (ret != 0) {
            LOG_CRITICAL(Common, "ftruncate failed with {}, are you out-of-memory?",
                         strerror(errno));
            throw std::bad_alloc{};
        }

        // Map the file handle which serves as our backing memory.
        backing_base = static_cast<u8*>(
            mmap(nullptr, backing_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
        if (backing_base == MAP_FAILED) {
            LOG_CRITICAL(Common, "mmap failed: {}", strerror(errno));
            throw std::bad_alloc{};
        }

        // Initialize our address space.
        virtual_map_base =
            reinterpret_cast<u8*>(mmap(nullptr, virtual_size, PROT_READ | PROT_WRITE,
        ...
        madvise(virtual_map_base, virtual_size, MADV_HUGEPAGE);
#endif

        free_manager.SetAddressSpace(virtual_map_base, virtual_size);
        good = true;
    }
    ...
    void Map(size_t virtual_offset, size_t host_offset, size_t length, MemoryPermission perms) {
        ...
        free_manager.AllocateBlock(virtual_offset, length);
        ...
        void* ret = mmap(reinterpret_cast<void*>(virtual_offset), length, flags,
    ...
    void Unmap(size_t virtual_offset, size_t length) {
        ...
        auto [merged_pointer, merged_size] = free_manager.FreeBlock(virtual_offset, length);
    ...
    void Protect(size_t virtual_offset, size_t length, bool read, bool write, bool execute) {
        ...
        int ret = mprotect(reinterpret_cast<void*>(virtual_offset), length, flags);
