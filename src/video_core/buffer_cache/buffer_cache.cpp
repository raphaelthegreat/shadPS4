...
BufferCache::BufferCache(const Vulkan::Instance& instance_, Vulkan::Scheduler& scheduler_,
    ...
    const auto null_buffer = slot_buffers[null_id].Handle();
...
void BufferCache::CopyBuffer(VAddr dst, VAddr src, u32 num_bytes, bool dst_gds, bool src_gds) {
    ...
    dst_buffer.Copy(src_buffer, std::move(region));
}
...
bool BufferCache::SynchronizeBuffer(Buffer& buffer, VAddr device_addr, u32 size, bool is_written,
    ...
    if (src_buffer) {
        buffer.Copy(*src_buffer, std::move(copies));
        TouchBuffer(buffer);
    }
    if (is_texel_buffer) {
...
Buffer* BufferCache::GetStagingBuffer(Buffer& buffer, std::span<vk::BufferCopy> copies,
    ...
    if (staging) {
        ...
        scheduler.DeferOperation([temp_buffer]() mutable { delete temp_buffer; });
        return temp_buffer;
    }
}
