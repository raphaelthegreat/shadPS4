...
void BufferCache::DownloadBufferMemory(Buffer& buffer, VAddr device_addr, u64 size, bool is_write) {
    ...
    if (preemptive_downloads.Intersects(page_addr, PageManager::PAGE_SIZE)) {
        preemptive_downloads.ForEachInRange(page_addr, PageManager::PAGE_SIZE, [&](VAddr begin, VAddr end, const PreemptiveDownload& download) {
            scheduler.Wait(download.done_tick);
            memory->TryWriteBacking(std::bit_cast<u8*>(download.device_addr), download.staging, download.size);
        });
...
void BufferCache::CommitPendingGpuRanges() {
    ...
    gpu_modified_ranges_pending.ForEach([&](VAddr begin, VAddr end) {
        memory_tracker->ForEachPreemptiveFlushPage(begin, end - begin, [&](VAddr page_addr) {
            ...
            download_copies[buffer_id].emplace_back(buffer.Offset(start_addr), total_size_bytes, size);
    ...
    if (!download_copies.empty()) {
        ...
        for (auto it = download_copies.begin(); it != download_copies.end(); ++it) {
            ...
            for (auto& copy : copies) {
                ...
                preemptive_downloads.Add(start_addr, copy.size, PreemptiveDownload{
