...
constexpr u64 BYTES_PER_PAGE = 1ULL << 12;
...
class Tracker {
public:
    void UpdatePagesCachedCount(VAddr addr, size_t size, bool is_cached) {
        //printf("called addr=0x%x size=0x%x\n", addr, size);
    }
};
...
public:
    ...
    void ChangeRegionState(u64 dirty_addr, u64 size) noexcept(type == Type::GPU) {
        ...
        IterateWords(dirty_addr - cpu_addr, size, [&](size_t index, u64 mask) {
            if constexpr (type == Type::CPU) {
                NotifyRasterizer<!is_dirty>(index, untracked_words[index], mask);
            }
