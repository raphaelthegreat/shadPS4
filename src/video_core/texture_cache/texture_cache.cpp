...
void GuestAlignmentFaultSignalHandler(int sig, siginfo_t* info, void* raw_context) {
...
TextureCache::TextureCache(const Vulkan::Instance& instance_, Vulkan::Scheduler& scheduler_)
    ...
    guest_access_fault.sa_sigaction = &GuestAlignmentFaultSignalHandler;
...
Image& TextureCache::FindDisplayBuffer(const Core::Libraries::VideoOut::BufferAttributeGroup& group,
                                       VAddr cpu_address) {
...
void TextureCache::RegisterImage(ImageId image_id) {
    ...
    ForEachPage(image.cpu_addr, image.guest_size_bytes,
                [this, image_id](u64 page) { page_table[page].push_front(image_id); });
}
...
void TextureCache::UnregisterImage(ImageId image_id) {
    ...
    ForEachPage(image.cpu_addr, image.guest_size_bytes, [this, image_id](u64 page) {
        ...
        image_ids.erase_after(std::prev(vector_it));
    });
...
void TextureCache::UpdatePagesCachedCount(VAddr addr, u64 size, s32 delta) {
    ...
    for (const auto& [range, count] : boost::make_iterator_range(range)) {
        ...
        if (delta > 0 && count == delta) {
            printf("Marking region as cached\n");
            mprotect((void*)interval_start_addr, interval_size, PROT_NONE);
            // memory.MarkRegionCached(interval_start_addr, interval_size, true);
        } else if (delta < 0 && count == -delta) {
            printf("Marking region as uncached\n");
            mprotect((void*)interval_start_addr, interval_size, PROT_READ | PROT_WRITE);
            // memory.MarkRegionCached(interval_start_addr, interval_size, false);
        } else {
            ASSERT(count >= 0);
        }
    }
